...
QUnit.test( "defaultMessage(), empty title is ignored", function( assert ) {
	...
	assert.equal( v.defaultMessage( $( "#username" )[ 0 ], { method: "required", parameters: true } ), "Este campo es requerido." );

	// Using the old way when we pass the name of a method as the second parameters.
	assert.equal( v.defaultMessage( $( "#username" )[ 0 ], "required" ), "Este campo es requerido." );
} );
...
QUnit.test( "works on contenteditable fields", function( assert ) {
	...
	assert.hasError( $( "#contenteditableRequiredInvalid" ), "Este campo es requerido." );
